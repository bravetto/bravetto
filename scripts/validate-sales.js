...
const required_patterns = [
  ...
  { name: "elevenlabs standard widget", pattern: /<elevenlabs-convai/ },
  {
    ...
    pattern: /elevenlabs\.io\/convai-widget\/index\.js/,
  },
  {
    name: "widget fixed position css",
    pattern: /elevenlabs-convai\s*\{\s*position:\s*fixed;/s,
  },
];

const forbidden_patterns = [
  {
    name: "ESM import of elevenlabs (causes microphone permission issues)",
    pattern:
      /import\s*\{\s*Conversation\s*\}\s*from\s*"https:\/\/esm\.sh\/@elevenlabs\/client"/,
  },
  {
    name: "page-level getUserMedia capture (duplicates widget microphone authority)",
    pattern: /getUserMedia\s*\(\s*\{\s*audio\s*:\s*true\s*\}\s*\)/,
  },
];
...
if (missing.length || forbidden_found.length) {
  ...
  if (missing.length)
    missing.forEach((p) => console.error("- Missing: " + p.name));
  if (forbidden_found.length)
    forbidden_found.forEach((p) => console.error("- Forbidden: " + p.name));
  process.exit(1);
}
